...
function getQuestions(){
    fetch('http://20.50.57.69:8080/koZnaZna/play').then(
        (response) => {
...
async function submitSelect(){
        ...
        const response = await fetch('http://20.50.57.69:8080/koZnaZna/submitQuestion', {
            method: 'POST',
            headers: {
...
async function finishGame(){
    const response = await fetch('http://20.50.57.69:8080/koZnaZna/finishGame', {
            method: 'PUT'
        });
        if (response.status !== 200) {
            console.log('Error: ' + response.status);
...
async function updateQuestionNumber(){
    ...
    const response = await fetch('http://20.50.57.69:8080/koZnaZna/nextQuestion', {
        method: 'PUT',
        headers: {
            'Content-Type': 'application/json'
        },
...
function getNumberOfPoints(){
     fetch('http://20.50.57.69:8080/koZnaZna/numberOfPoints/'+gameId).then(
        (response) => {
                if (response.status !== 200) {
                    console.log('Error: ' + response.status);
                    return;
